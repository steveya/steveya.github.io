...
Equation 
ef{eq:xdist} implies that $X_t^2 \sim s_t^2 \chi_1^2$ where $\chi_1^2$ is a Chi-squared distribution with $1$ degree of freedom. The expectation of $X_t$ is $s_t^2$, leading us to the distribution of $Y_t$
...
Similarly, the asymptotic Sharpe ratio, a measure of risk-adjusted return, is shown to be directly proportional to the mean reversion speed $\\theta$ while remaining unaffected by $\\sigma$. Equation 
ef{eq:sr} shows that $\\theta$ influences the volatility of the strategy's PnLs. Meanwhile, an increase in mean reversion speed leads to reduced volatility of average PnLs, subsequently enhancing the Sharpe ratio.  
