...
date: 2024-08-10
categories: [Study Notes, Quantitative Finance, Short Rate Models]
---
...
1. Start with the general bond pricing equation:
 $P(t,T) = \mathbb{E}_t^Q\[\exp\left(-\int_t^T r_s ds\right)\]$
...
$\mathbb{E}\[\exp\left(-X\right)\] = \exp\left(-m + \frac{1}{2}v\right)$
...
Now to get $\mathbb{E}_t^Q\[\int_t^T r_s ds\right]$ and $\text{Var}_t^Q\[\int_t^T r_s ds\right]$, we need to again integrate $\int_t^T r_s ds$, which turns out to be
...
We can compute the $\mathbb{E}_t^Q\[\int_t^T r_s ds\right]$ and $\text{Var}_t^Q\[\int_t^T r_s ds\right]$ by taking the expectation of the drift and diffusion terms, respectively:

$$ 
\begin{equation}
\begin{aligned}
\mathbb{E}_t^Q\[\int_t^T r_s ds\] &= r_t \left(\frac{1 - e^{-\kappa\left(T-t\right)}}{\kappa}\right) + \theta \left(\left(T-t\right) - \frac{1 - e^{-\kappa\left(T-t\right)}}{\kappa}\right) \\
\text{Var}_t^Q\[\int_t^T r_s ds\] &= \text{Var}\[\frac{\sigma}{\kappa}\int_t^T\left(1-e^{-\kappa\left(T-u\right)}\right)dW_u\] \\
...
&= \frac{\sigma^2\left(T-t\right)}{\kappa^2} + \frac{\sigma^2}{2\kappa^3}\[1-e^{-2\kappa\left(T-u\right)}\] - \frac{2\sigma^2}{\kappa^3}\[1 - e^{-\kappa\left(T-u\right)}\] \\
&= \frac{\sigma^2}{2\kappa^3}\left(2\kappa\left(T-t\right) - 3 - e^{2\kappa\left(T-t\right)} + 4e^{-2\kappa\left(T-t\right)}\right) \\
\end{aligned}
\end{equation}
$$ 

5. Substitute the expression for $\mathbb{E}_t^Q\[\int_t^T r_s ds\right]$ and $\text{Var}_t^Q\[\int_t^T r_s ds\right]$ into the bond price formula, and let $\tau = T-t$, and after some tedious algebra, we get

$$ 
\begin{equation}
\begin{aligned}
P(t,T) &= \mathbb{E}_t^Q\[\exp\left(-\int_t^T r_s ds\right)\] \\
&= \exp\left[-\mathbb{E}_t^Q\[\int_t^T r_s ds\]+\frac{1}{2}\text{Var}_t^Q\[\int_t^T r_s ds\]\right] \\
&= \exp\left[-r_t \left(\frac{1 - e^{-\kappa\tau}}{\kappa}\right) - \theta \left(\tau - \frac{1 - e^{-\kappa\tau}}{\kappa}\right)+ \frac{\sigma^2}{4\kappa^3}\left(2\kappa\tau - 3 - e^{2\kappa\tau} + 4e^{-2\kappa\tau}\right)\]\\
&= \exp\left[-r_t \left(\frac{1 - e^{-\kappa\tau}}{\kappa}\right) + \left(\theta -\frac{\sigma^2}{\kappa^2}\right)\left[\frac{1-e^{-\kappa\tau}}{\kappa}-\tau\] - \frac{\sigma^2}{4\kappa}\left[\frac{1 - e^{-\kappa\tau}}{\kappa}\right]^2\]\\
\end{aligned}
\end{equation}
$$ 
...
\begin{equation}
\begin{aligned}
\mathbb{E}_t^Q\[r_s \vert r_t\] &=r_t e^{-\kappa \left(s-t\right)} + \theta \left(1 - e^{-\kappa\left(s-t\right)}\right) \\
\text{Var}_t^Q\[r_s \vert r_t\] &= \text{Var}\[\sigma \int_t^s e^{-\kappa \left(u-t\right)} dW_u\] \\
...
A(t,T) &= \exp\left[\[\left(\theta -\frac{\sigma^2}{\kappa^2}\right)\left[\frac{1-e^{-\kappa\tau}}{\kappa}-\tau\] - \frac{\sigma^2}{4\kappa}\left[\frac{1 - e^{-\kappa\tau}}{\kappa}\right]^2\]\right] \\
...
&= \left[\frac{y_t - r_t}{\tau} - \frac{1}{2}\frac{B^2}{\tau}\sigma^2\] dt + \frac{B}{\tau} \sigma dW_t \\
\end{aligned}
\end{equation}
$$ 
...
$dy = \left[\frac{y_t - r_t}{\tau} - \frac{1}{2}\tau\sigma^2\] dt + \sigma dW_t$ 
